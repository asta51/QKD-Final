from security_monitor import SecurityMonitor, calculate_entropy
import socket
import threading
import base64
...
class SecureServer:
    ...
    def _handle_client(self, conn, addr):
        try:
            ...
            # Calculate key entropy and log
            entropy = calculate_entropy(final_key_bits)
            self.monitor.log_communication(
                packet_size=len(key_bytes),
                key_entropy=entropy
            )
            ...
            while self.running:
                try:
                    ...
                    # Log outgoing packet
                    self.monitor.log_communication(
                        packet_size=len(encrypted_response)
                    )

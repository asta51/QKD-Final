...
class SecureServer:
    ...
    def _combine_keys(self, key_bb84, key_e91):
        """
        Combine keys using Brahmagupta mapping and Ramanujan modular mapping
        """
        # Convert 8-bit chunks to integers
        ...
        # Apply Ramanujan modular mapping for privacy amplification
        final_bits = ramanujan_modular_mapping(combined_bits)
        return final_bits[:len(key_bb84)]  # trim to original key length
    ...
    def _handle_client(self, conn, addr):
        try:
            ...
            # Generate BB84 & E91 keys
            key_bb84 = self.bb84.generate_key()
            key_e91 = self.e91.generate_key()
            
            # Combine using Brahmagupta + Ramanujan mapping
            final_key_bits = self._combine_keys(key_bb84, key_e91)
